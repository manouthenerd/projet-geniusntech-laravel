    ...
    <div class="max-w-[85rem] px-4 py-10 sm:px-6 lg:px-8 lg:py-14 mx-auto">
        <div class="mx-auto max-w-2xl">
            ...
            <div
                ...
                <form method="post" action="{{route('dashboard.services.show', ['service' => $service->id])}}" class="p-2 space-y-4" enctype="multipart/form-data">
                    @method('PUT')
                    @csrf
                    <div class="mb-4 sm:mb-8">
                        <label for="hs-feedback-post-comment-name-1"
                            ...
                            Nom du service
                            @if (request()->routeIs("dashboard.services.show", $service->id))
                                <x-ui.label-tag text="En lecture uniquement" />
                            @endif
                        </label>
                        <input @readonly(request()->routeIs("dashboard.services.show", $service->id)) name="title" value="{{ $service->title }}" type="text"
                    ...
                    <div class="mb-4 sm:mb-8">
                        ...
                        <input @readonly(request()->routeIs("dashboard.services.show", $service->id)) name="summary" value="{{ $service->summary }}" type="text"
                            id="hs-feedback-post-comment-name-1"
                            class="border py-2.5 sm:py-3 px-4 block w-full border-slate-300 rounded-lg sm:text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600"
                    ...
                    @if (request()->routeIs("dashboard.services.edit", $service->id))
                    ...
                    @if (request()->routeIs("dashboard.services.show", $service->id))
                        <div class="mt-6 grid grid-cols-2 space-x-4 gap-2">
                            ...
                            <flux:button :href="route('dashboard.services.edit', $service->id)"
