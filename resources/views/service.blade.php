    ...
    <section class="mx-8 p-2 rounded">
        ...
        <ul id="services" class="grid grid-cols-2 my-2 max-[830px]:grid-cols-1 w-full space-y-4 space-x-4">
            ...
            @foreach ($next_services as $service)
                <li data-aos="fade-up" data-aos-duration="1200" data-aos-delay="{{ $service->id * 1000 }}"
                    class="grid gap-2 p-2 rounded bg-white shadow-sm max-h-auto">

                    <a class="space-y-2" href="/services/{{ $service->identifier }}" id="contents">
                        <div>
