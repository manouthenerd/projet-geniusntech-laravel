            ...
            <div
                ...
                <form method="post" action="{{ route('dashboard.save-blog', ['blog' => $id]) }}" class="p-2 space-y-4" enctype="multipart/form-data">
                    ...
                    <div class="mb-4 sm:mb-8">
                        ...
                        <input @readonly(request()->is("dashboard/blogs/$id")) wire:model='title' name="title" type="text"
                            id="hs-feedback-post-comment-name-1" value="{{ $article['title'] }}"
                    ...
                    <div class="mb-4 sm:mb-8">
                        ...
                        <input wire:model='category' value="{{ $article['category'] }}" @readonly(request()->is("dashboard/blogs/$id"))
                            name="category" placeholder="Technologie, éducation, divertissement" type="text"
                            id="category"
                            class="border bg-white text-zinc-600 py-2.5 sm:py-3 px-4 block w-full border-slate-300 rounded-lg sm:text-sm focus:border-blue-500 focus:ring-blue-500 disabled:opacity-50 disabled:pointer-events-none dark:bg-neutral-900 dark:border-neutral-700 dark:text-neutral-400 dark:placeholder-neutral-500 dark:focus:ring-neutral-600">
                    ...
                    @if (request()->is("dashboard/blogs/$id/edit"))
                        <div id="upload-box" style="overflow: hidden; text-overflow: ellipsis">
                            <flux:input id="upload-input" type="file" wire:model='image' name='image'
                                label="Sélectionner l'image d'illustration" class="text-zinc-400" />
                        </div>
                    @endif

                    <label class="block mb-2 text-zinc-400 text-sm font-medium dark:text-white">Image associée à
                        l'article</label>
                    ...
                    @if (request()->is("dashboard/blogs/$id"))
                        <div class="mt-6 grid grid-cols-2 space-x-4 gap-2">
                            ...
                            <flux:button href="/dashboard/blogs/{{ $id }}/edit"
